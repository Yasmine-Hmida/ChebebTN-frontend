...
const Edit = () => {
  ...
  const [title, setTitle] = useState(parsedJob.title);
  const [company, setCompany] = useState(parsedJob.company);
  const [description, setDescription] = useState(parsedJob.description);
  const [salary, setSalary] = useState(formatSalary(parsedJob.salary));
  const [location, setLocation] = useState(parsedJob.location);
  const [jobType, setJobType] = useState(parsedJob.jobType);
  const [status, setStatus] = useState(parsedJob.status);
  const [experienceLevel, setExperienceLevel] = useState(parsedJob.experienceLevel);
  const [skills, setSkills] = useState(parsedJob.skills);
  ...
  return (
    <ScrollView  style={style.container} contentContainerStyle={{ justifyContent: 'center', alignItems: 'center' }}>
      <Text style={style.title}>Edit Page:</Text>
      ...
      >
        <Text style={style.addButtonText}>Edit job</Text>
      </TouchableOpacity>
    </ScrollView>
  );
};
